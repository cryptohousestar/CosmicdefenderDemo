...
const LogManager = {
    ...
    log(type, message, data = null) {
        ...
        if (this.logTimestamps[key] && now - this.logTimestamps[key] < this.minInterval) {
            return;
        }
...
const PerformanceOptimizer = {
    ...
    detectGPU() {
        ...
        if (debugInfo) {
            ...
            return renderer !== 'ANGLE (Intel, Intel(R) HD Graphics 4000 Direct3D11 vs_4_0 ps_4_0)';
        }
        return true;
    },
    ...
    determineOptimalSettings() {
        ...
        if (savedMode && ['low', 'medium', 'high', 'ultra', 'extreme'].includes(savedMode)) {
    ...
    updatePerformanceMetrics(deltaTime) {
        ...
        if (this.deviceCapabilities.frameCount % 60 === 0) {
            this.deviceCapabilities.fps = Math.round(1000 / this.deviceCapabilities.averageFrameTime);
            this.deviceCapabilities.averageFrameTime = deltaTime;
            
            // Solo degradar si el rendimiento es muy bajo (más tolerante)
            if (this.deviceCapabilities.fps < 20 && this.deviceCapabilities.performanceMode !== 'low') {
                this.downgradePerformance();
            } else if (this.deviceCapabilities.fps > 100 && this.deviceCapabilities.performanceMode !== 'extreme') {
    ...
    downgradePerformance() {
        ...
        if (currentIndex < modes.length - 1) {
            ...
            console.log('Performance downgraded to:', this.deviceCapabilities.performanceMode);
        }
    },
    ...
    upgradePerformance() {
        ...
        if (currentIndex > 0) {
            this.deviceCapabilities.performanceMode = modes[currentIndex - 1];
            console.log('Performance upgraded to:', this.deviceCapabilities.performanceMode);
        }
    },
...
const DEFAULT_WIDGET_POSITIONS = {
    minimapContainer: { x: window.innerWidth - 170, y: 20 }, // Esquina superior derecha
    playerStats: { x: 20, y: 20 }, // Izquierda
    ...
    skillsWidget: { x: window.innerWidth / 2 - 150, y: window.innerHeight - 80 },
};
...
class CosmicDefender {
    constructor() {
        ...
        this.earthImage.onload = () => { console.log('Imagen de la Tierra cargada correctamente'); };
        this.earthImage.onerror = () => {
            alert('No se pudo cargar la imagen de la Tierra. Verifica la ruta: recursos_graficos/tierra(1).png');
        ...
        this.equipment = {
            weapon: { name: "Basic Laser Gun", damage: 1, icon: "🔫", rarity: "Common", tier: 1 },
            shield: { name: "Basic Shield", defense: 0, icon: "🛡️", rarity: "Common", tier: 1 },
            engine: { name: "Basic Engine", speed: 0, icon: "⚡", rarity: "Common", tier: 1 }
        };

        // Sistema de logros
        this.achievementsList = [
            { id: "firstKill", name: "Primera Sangre", description: "Mata tu primer enemigo", reward: 50, unlocked: false },
            { id: "killStreak5", name: "Máquina de Matar", description: "Mata 5 enemigos seguidos", reward: 100, unlocked: false },
            { id: "killStreak10", name: "Asesino", description: "Mata 10 enemigos seguidos", reward: 200, unlocked: false },
            { id: "survive5min", name: "Sobreviviente", description: "Sobrevive 5 minutos", reward: 150, unlocked: false },
            { id: "level5", name: "Veterano", description: "Alcanza nivel 5", reward: 300, unlocked: false },
            { id: "level10", name: "Experto", description: "Alcanza nivel 10", reward: 500, unlocked: false },
            { id: "completePlanet", name: "Conquistador", description: "Completa un planeta", reward: 1000, unlocked: false },
            { id: "richPlayer", name: "Rico", description: "Acumula 1000 monedas", reward: 200, unlocked: false }
        ];

        // Sistema de habilidades
        this.skills = {
            q: { name: "Ráfaga Rápida", icon: "⚡", cooldown: 0, maxCooldown: 5000, description: "Dispara 5 proyectiles rápidamente" },
            e: { name: "Escudo Temporal", icon: "🛡️", cooldown: 0, maxCooldown: 8000, description: "Escudo que bloquea daño por 3 segundos" },
            r: { name: "Turbo Boost", icon: "🚀", cooldown: 0, maxCooldown: 6000, description: "Aumenta velocidad por 5 segundos" },
            t: { name: "Mega Disparo", icon: "💥", cooldown: 0, maxCooldown: 10000, description: "Dispara un proyectil masivo" },
            f: { name: "Campo de Fuerza", icon: "🌀", cooldown: 0, maxCooldown: 15000, description: "Empuja enemigos lejos" }
        };
        
        // Item database inspired by RotMG
        this.itemDatabase = {
            weapons: [
                { name: "Laser Pistol", damage: 1, icon: "🔫", rarity: "Common", tier: 1 },
                { name: "Plasma Rifle", damage: 3, icon: "🔫", rarity: "Uncommon", tier: 2 },
                { name: "Quantum Cannon", damage: 5, icon: "🔫", rarity: "Rare", tier: 3 },
                { name: "Void Blaster", damage: 8, icon: "🔫", rarity: "Epic", tier: 4 },
                { name: "Cosmic Destroyer", damage: 12, icon: "🔫", rarity: "Legendary", tier: 5 }
            ],
            shields: [
                { name: "Basic Shield", defense: 0, icon: "🛡️", rarity: "Common", tier: 1 },
                { name: "Reinforced Shield", defense: 2, icon: "🛡️", rarity: "Uncommon", tier: 2 },
                { name: "Energy Barrier", defense: 4, icon: "🛡️", rarity: "Rare", tier: 3 },
                { name: "Quantum Shield", defense: 6, icon: "🛡️", rarity: "Epic", tier: 4 },
                { name: "Cosmic Aegis", defense: 10, icon: "🛡️", rarity: "Legendary", tier: 5 }
            ],
            engines: [
                { name: "Basic Engine", speed: 0, icon: "⚡", rarity: "Common", tier: 1 },
                { name: "Turbo Engine", speed: 1, icon: "⚡", rarity: "Uncommon", tier: 2 },
                { name: "Quantum Drive", speed: 2, icon: "⚡", rarity: "Rare", tier: 3 },
                { name: "Warp Engine", speed: 3, icon: "⚡", rarity: "Epic", tier: 4 },
                { name: "Cosmic Thruster", speed: 5, icon: "⚡", rarity: "Legendary", tier: 5 }
            ]
        };
        ...
        this.skillKeyMap = JSON.parse(localStorage.getItem('skillKeyMap') || JSON.stringify(this.DEFAULT_SKILL_KEYMAP));
    ...
    async connectToServer(serverId, sessionToken, walletAddress) {
        try {
            ...
            const success = await this.gameIntegration.initialize(serverId, sessionToken, walletAddress);
            ...
            if (success) {
                ...
                if (this.gameIntegration.canPlay()) {
                    ...
                    console.log('❌ User cannot play:', this.gameIntegration.getPlayabilityMessage());
                    this.showPlayabilityError(this.gameIntegration.getPlayabilityMessage());
                }
            } else {
    ...
    setupEventListeners() {
        ...
        if (selectClanBtn && !selectClanBtn._listenerAdded) {
            selectClanBtn.addEventListener('click', () => { this.showClanSelector(); });
        ...
        if (goToDungeonsBtn && !goToDungeonsBtn._listenerAdded) {
            goToDungeonsBtn.addEventListener('click', () => { window.location.href = 'mercury.html'; });
        ...
        if (goToMarketBtn && !goToMarketBtn._listenerAdded) {
            goToMarketBtn.addEventListener('click', () => { window.open('market.html', '_blank'); });
        ...
        if (addCoinsBtn && !addCoinsBtn._listenerAdded) {
            addCoinsBtn.addEventListener('click', () => { this.addTestCoins(100); });
        ...
        if (debugClanBtn && !debugClanBtn._listenerAdded) {
            debugClanBtn.addEventListener('click', () => { this.debugClanSelector(); });
        ...
        if (backToMenuFromClanBtn && !backToMenuFromClanBtn._listenerAdded) {
            backToMenuFromClanBtn.addEventListener('click', () => { this.showMenu(); });
        ...
        if (backToClanSelectorBtn && !backToClanSelectorBtn._listenerAdded) {
            backToClanSelectorBtn.addEventListener('click', () => { this.showClanSelector(); });
    ...
    addTestCoins(amount = 100) {
        ...
        console.log('Monedas actuales en localStorage:', localStorage.getItem('coins'));
        ...
        console.log('Monedas guardadas en localStorage:', localStorage.getItem('coins'));
        ...
        alert(`Se agregaron ${amount} monedas. Total: ${newCoins}\nVerifica la consola para más detalles.`);
    }
    ...
    debugClanSelector() {
        ...
        if (clanSelector) {
            ...
            console.log('Clan selector visibility:', window.getComputedStyle(clanSelector).visibility);
            console.log('Clan selector z-index:', window.getComputedStyle(clanSelector).zIndex);
        }
    ...
    registerAllGameWidgets() {
        ...
        // Panel de rendimiento eliminado
    ...
    registerGameWidgets() {
        ...
        this.registerWidget('playerStats', '.player-stats', { x: 10, y: 10 }, 'Estadísticas del Jugador');
        
        // Widgets de botones de tienda (se crean dinámicamente)
        this.registerWidget('tiendaBtn', '#tiendaBtn', { x: window.innerWidth / 2 - 100, y: 80 }, 'Botón de Tienda', true);
        
        // Widgets de notificaciones (futuros)
        this.registerWidget('notifications', null, { x: 10, y: window.innerHeight - 100 }, 'Notificaciones', true);
        
        // Crear contenedor para el minimapa existente
        this.createMinimapContainer();
        
        // Forzar actualización de posición del minimapa
    ...
    registerDynamicWidgets() {
        ...
        setTimeout(() => {
            ...
            this.registerWidget('tiendaBtn', '#tiendaBtn', { x: window.innerWidth / 2 - 100, y: 80 }, 'Botón de Tienda');
    ...
    createMinimapContainer() {
        ...
        // Actualizar posición a esquina superior derecha
        minimapDiv.style.left = (window.innerWidth - 170) + 'px';
        ...
        // Registrar el contenedor como widget con posición actualizada
        this.registerWidget('minimapContainer', '#minimapContainer', { x: window.innerWidth - 170, y: 20 }, 'Minimapa', true);
    }
    
    updateMinimapPosition() {
        // Forzar actualización de posición del minimapa a esquina superior derecha
        const minimapDiv = document.getElementById('minimapContainer');
        if (minimapDiv) {
            minimapDiv.style.left = (window.innerWidth - 170) + 'px';
            minimapDiv.style.top = '20px';
            console.log('Minimapa movido a esquina superior derecha:', minimapDiv.style.left, minimapDiv.style.top);
        }
    }
    ...
    createWidgetSystem() {
        ...
        this.addWidget(configButton, 'widgetConfigButton', { x: 20, y: 20 }, 'Botón Configurar UI');
        ...
        this.addWidget(skillsWidget, 'skillsWidget', DEFAULT_WIDGET_POSITIONS.skillsWidget, 'Skills');
    }
    ...
    createWidgetInstructions() {
        ...
        // Agregar al sistema de widgets
        this.addWidget(instructions, 'widgetInstructions', { x: 20, y: window.innerHeight - 200 }, 'Instrucciones de Widgets');
    }
    ...
    toggleWidgetEditMode() {
        ...
        if (this.isWidgetEditMode) {
            ...
            if (!document.getElementById('saveWidgetsButton')) {
                ...
                this.addWidget(saveBtn, 'saveWidgetsButton', { x: 320, y: 20 }, 'Botón Guardar');
            }
    ...
    createResetButton() {
        ...
        // Agregar al sistema de widgets
        this.addWidget(resetButton, 'resetWidgetsButton', { x: 200, y: 20 }, 'Botón Resetear');
    }
    ...
    makeWidgetsDraggable() {
        document.querySelectorAll('.widget').forEach(widget => {
            ...
            widget.removeEventListener('mousedown', (e) => this.startWidgetDrag(e, widget));
            
            // Agregar eventos de drag
            widget.addEventListener('mousedown', (e) => this.startWidgetDrag(e, widget));
            
            LogManager.log('info', `Widget ${widget.dataset.widgetName || widget.id} hecho arrastrable`);
        });
        
        LogManager.log('info', `Total de widgets arrastrables: ${document.querySelectorAll('.widget').length}`);
    }
    ...
    startWidgetDrag(e, widget) {
        ...
        LogManager.log('info', `Iniciando arrastre de widget: ${widget.dataset.widgetName || widget.id}`);
    }
    ...
    stopWidgetDrag = () => {
        if (this.draggedWidget) {
            ...
            LogManager.log('info', `Finalizado arrastre de widget: ${this.draggedWidget.dataset.widgetName || this.draggedWidget.id}`);
            this.draggedWidget = null;
        }
    ...
    loadWidgetPositions() {
        ...
        this.widgets.forEach(widget => {
            ...
            if (!useDefault && positions[widget.type] && typeof positions[widget.type].visible !== 'undefined') {
                widget.element.style.display = positions[widget.type].visible ? 'block' : 'none';
            } else {
                // Por defecto: solo estadísticas del jugador y minimapa visibles
                if (widget.type === 'playerStats' || widget.type === 'minimapContainer') {
                    widget.element.style.display = 'block';
                } else {
                    // Todos los demás widgets ocultos por defecto
                    widget.element.style.display = 'none';
                }
            }
        });
        LogManager.log('info', useDefault ? 'Se usaron posiciones por defecto para widgets' : 'Posiciones y visibilidad de widgets cargadas de localStorage');
    }
    ...
    createWidgetControlPanel() {
        ...
        document.getElementById('showAllWidgets').onclick = () => this.showAllWidgets();
        document.getElementById('hideAllWidgets').onclick = () => this.hideAllWidgets();
        document.getElementById('resetAllWidgets').onclick = () => this.resetAllWidgets();
        
        // Agregar al sistema de widgets
        this.addWidget(controlPanel, 'widgetControlPanel', { x: 20, y: 80 }, 'Control de Widgets');
    }
    ...
    toggleWidgetVisibility(widgetType) {
        ...
        LogManager.log('info', `Widget ${widgetType} ${currentState.visible ? 'mostrado' : 'oculto'}`);
    }
    ...
    setupDefaultWidgets() {
        ...
        // Luego, mostrar solo los widgets deseados: estadísticas del jugador y minimapa
        const defaultWidgets = ['playerStats', 'minimapContainer'];
        this.widgets.forEach(widget => {
            if (defaultWidgets.includes(widget.type)) {
                ...
                // Asegurar que todos los demás estén ocultos
                widget.element.style.display = 'none';
                if (this.widgetStates[widget.type]) {
                    this.widgetStates[widget.type].visible = false;
                }
        ...
        this.saveWidgetStates();
        LogManager.log('info', 'Visibilidad de widgets por defecto aplicada - Solo estadísticas del jugador y minimapa visibles.');
    }
    ...
    forceWidgetDraggability() {
        ...
        problemWidgets.forEach(widgetInfo => {
            ...
            if (element && !element.classList.contains('widget')) {
                ...
                if (!existingWidget) {
                    ...
                    this.addWidget(element, widgetInfo.id || widgetInfo.selector, 
                                 { x: rect.left, y: rect.top }, widgetInfo.name);
                }
        ...
        console.log(`Total de widgets forzados: ${document.querySelectorAll('.widget').length}`);
    }
    ...
    removeUnusedWidgets() {
        ...
        this.widgets = this.widgets.filter(widget => 
            widget.type !== 'inventoryPanel' && 
            widget.type !== 'controlsInfo'
        );
    ...
    debugWidgetMovement() {
        ...
        allWidgets.forEach((widget, index) => {
            const widgetName = widget.dataset.widgetName || widget.id || `Widget ${index}`;
    ...
    fixPlayerStatsWidget() {
        ...
        if (playerStats) {
            ...
            const existingWidget = this.widgets.find(w => w.element === playerStats);
            if (!existingWidget) {
                const rect = playerStats.getBoundingClientRect();
                this.addWidget(playerStats, 'playerStats', { x: rect.left, y: rect.top }, 'Estadísticas del Jugador');
            }
            
            // Forzar event listener de arrastre
            playerStats.removeEventListener('mousedown', (e) => this.startWidgetDrag(e, playerStats));
            if (this.isWidgetEditMode) {
                playerStats.addEventListener('mousedown', (e) => this.startWidgetDrag(e, playerStats));
    ...
    updatePlayerSpeed() {
        if (this.player) {
            ...
            LogManager.log('info', `Velocidad actualizada: ${this.player.speed.toFixed(1)} (${this.speedMultiplier.toFixed(1)}x)`);
        }
    }
    ...
    startGameWithIntegration() {
        ...
        let spawnX, spawnY;
        do {
            spawnX = this.centralSafeZone.x + this.centralSafeZone.width / 2 + (Math.random() - 0.5) * 100;
            spawnY = this.centralSafeZone.y + this.centralSafeZone.height / 2 + (Math.random() - 0.5) * 100;
        } while (spawnX < this.centralSafeZone.x + 50 || spawnX > this.centralSafeZone.x + this.centralSafeZone.width - 50 || 
                 spawnY < this.centralSafeZone.y + 50 || spawnY > this.centralSafeZone.y + this.centralSafeZone.height - 50);
        ...
        if (equippedWeapon) {
            ...
            this.player.fireRate = equippedWeapon.stats.fireRate || this.player.fireRate;
        }
    ...
    startGameStandalone() {
        ...
        const playerBando = this.playerBando !== null ? this.bandos[this.playerBando] : null;
    ...
    updateUI() {
        ...
        if (hpElement) hpElement.textContent = `${this.player.health}/${this.player.maxHealth}`;
        ...
        const engineSpeed = this.equipment.engine && this.equipment.engine.speed ? this.equipment.engine.speed : 0;
        ...
        if (hpDisplayElement) hpDisplayElement.textContent = `${this.player.health}/${this.player.maxHealth}`;
        ...
        if ((this.isPlayerInSafeZone(this.player) &&
    ...
    updatePerformanceUI() {
        // Panel de rendimiento eliminado - no se muestra
        // Solo mantener la funcionalidad interna sin UI
    }
    
    togglePerformanceMode() {
        const modes = ['low', 'medium', 'high', 'ultra', 'extreme'];
        const currentIndex = modes.indexOf(PerformanceOptimizer.deviceCapabilities.performanceMode);
    ...
    loadEquipmentFromMarket() {
        ...
        if (equippedData) {
            ...
            if (equipped.ship) {
                ...
                this.player.speed = (equipped.ship.stats.speed * this.speedMultiplier) * 10; // Velocidad con multiplicador de desarrollo x10
            ...
            console.error("No ship equipped, but game started. Player will not be a ghost, but might not be fully functional.");
            return;
            }
            ...
            if (equipped.shield) {
                ...
                const baseHealth = this.equipment.ship ? this.equipment.ship.stats.hp : this.player.maxHealth;
                const shieldBonus = Math.floor(baseHealth * (equipped.shield.stats.lifePercent / 100));
    ...
    updatePlayer() {
        ...
        // Update player angle to point towards mouse (do this before movement)
        ...
        if (this.keys['w'] || this.keys['arrowup']) {
            newX = Math.max(0, Math.min(this.worldSize - this.player.width, newX + Math.cos(this.player.angle) * totalSpeed));
            newY = Math.max(0, Math.min(this.worldSize - this.player.height, newY + Math.sin(this.player.angle) * totalSpeed));
        }
        // Strafe (A/← y D/→)
        if (this.keys['a'] || this.keys['arrowleft']) {
            const strafeAngle = this.player.angle - Math.PI / 2;
            newX = Math.max(0, Math.min(this.worldSize - this.player.width, newX + Math.cos(strafeAngle) * totalSpeed));
            newY = Math.max(0, Math.min(this.worldSize - this.player.height, newY + Math.sin(strafeAngle) * totalSpeed));
        }
        ...
        LogManager.log('debug', '[Disparo P2] updatePlayer: Verificando disparo.', { isGhost: this.player.isGhost, mouseClicked: this.mouse.clicked, fireRateCooldown: Date.now() - this.lastShot > this.getFireRate() * 1000 });
        if (this.mouse.clicked && Date.now() - this.lastShot > this.getFireRate() * 1000) {
            this.shoot();
    ...
    getFireRate() {
        if (!this.equipment.weapon || !this.equipment.weapon.stats) return this.player.fireRate;
        
        // Calculate fire rate based on equipped weapon
        let fireRate = this.player.fireRate;
        if (this.equipment.weapon.stats.fireRate === 'muy alta') {
            fireRate = this.player.fireRate / 3;
        } else if (this.equipment.weapon.stats.fireRate === 'alta') {
            fireRate = this.player.fireRate / 2;
        } else if (this.equipment.weapon.stats.fireRate === 'media') {
            fireRate = this.player.fireRate / 1.5;
        }
    ...
    updateBots() {
        ...
        if (this.useWebWorkers && this.botWorker) {
            ...
            this.botWorker.postMessage({
                ...
                data: {
                    bots: this.bots,
                    worldData: { // Only send dynamic data
        ...
        if (player.isHuman) {
            ...
            for (let i = 0; i < 50; i++) {
                this.particles.push({
                    ...
                    color: player.isBoss ? '#ff00ff' : (player.isElite ? '#FF6B35' : '#ffff00')
                });
            }

            const clan = this.clans[player.clan];
            const safeZone = this.clanSafeZones.find(zone => zone.clan === player.clan);
            ...
            setTimeout(() => {
                this.addDamageNumber('RESPAWN', spawnX + player.width / 2, spawnY - 20, '#00ff00');
            }, 1000);
        }
    }
    ...
    shoot() {
        LogManager.log('debug', '[Disparo P3] shoot(): Creando y añadiendo bala al array.');
        const weaponDamage = this.equipment.weapon && this.equipment.weapon.damage ? this.equipment.weapon.damage : 0;
        const totalDamage = this.player.damage + weaponDamage;
    ...
    updateEnemies() {
        this.enemies.forEach((enemy, index) => {
            const distanceToPlayer = this.getDistance(enemy.x, enemy.y, this.player.x, this.player.y);
            ...
            if (enemy.state === 'chase') {
                ...
                if (distanceToPlayer < 200 && Date.now() - enemy.lastShot > enemy.fireRate * 1000) {
            ...
            this.obstacles.forEach(obstacle => {
                if (this.checkCollision(enemy, obstacle)) {
                    ...
                    const overlapX = Math.min(enemy.x + enemy.width - obstacle.x, obstacle.x + obstacle.width - enemy.x);
                    const overlapY = Math.min(enemy.y + enemy.height - obstacle.y, obstacle.y + obstacle.height - enemy.y);
    ...
    updateBullets() {
        ...
        this.bullets.forEach((bullet, bulletIndex) => {
            LogManager.log('debug', '[Disparo P4] updateBullets: Actualizando posición de la bala.', { bulletX: bullet.x, bulletY: bullet.y });
                    ...
                    if (player.health > 0) {
                        ...
                        if (shooterClan !== null && player.clan !== shooterClan) {
                            ...
                            if (!isInSafeZone && this.checkCollision(bullet, player)) {
                                ...
                                // Add damage number
                                this.addDamageNumber(finalDamage, player.x + player.width / 2, player.y + player.height / 2, '#ff0000');
                                
                                // Si el jugador murió, dropear monedas solo si el último hit fue del jugador
                                if (player.health <= 0) {
                                    if (player.lastDamageFrom === 'player') {
                                        this.dropCoins(player.x + player.width / 2, player.y + player.height / 2, 'bot', player.isElite);
                                        console.log(`💰 ${player.name} ${player.isElite ? '(ELITE)' : ''} dropeó monedas (último hit: jugador)`);
                                    } else {
                                        console.log(`💀 ${player.name} ${player.isElite ? '(ELITE)' : ''} murió (último hit: ${player.lastDamageFrom})`);
                                    }
                                }
        ...
        this.enemyBullets.forEach((bullet, bulletIndex) => {
            ...
            if (this.checkCollision(bullet, this.player)) {
                ...
                if (!isInSafeZone && !this.player.invincible) {
                    const actualDamage = Math.max(1, bullet.damage - this.equipment.shield.defense);
                    this.player.health -= actualDamage;
                    
                    // Add damage number for player
                    this.addDamageNumber(actualDamage, this.player.x + this.player.width / 2, this.player.y + this.player.height / 2, '#ff0000');
    ...
    populateSkillTree() {
        ...
        const combatSkillList = [
            { name: 'Ráfaga Rápida', description: 'Dispara 5 proyectiles rápidamente', cost: 2, icon: '⚡' },
            { name: 'Mega Disparo', description: 'Dispara un proyectil masivo', cost: 3, icon: '💥' },
            { name: 'Campo de Fuerza', description: 'Empuja enemigos lejos', cost: 4, icon: '🌀' }
        ];

        // Habilidades defensivas
        const defenseSkillList = [
            { name: 'Escudo Temporal', description: 'Escudo que bloquea daño por 3 segundos', cost: 2, icon: '🛡️' },
            { name: 'Turbo Boost', description: 'Aumenta velocidad por 5 segundos', cost: 3, icon: '🚀' },
            { name: 'Regeneración', description: 'Regenera 1 HP por segundo', cost: 5, icon: '💚' }
        ];

        combatSkills.innerHTML = combatSkillList.map(skill => `
        ...
        defenseSkills.innerHTML = defenseSkillList.map(skill => `
    ...
    drawSafeZone() {
        ...
        this.clanSafeZones.forEach(safeZone => {
            ...
            this.ctx.fillRect(screenPos.x, screenPos.y, safeZone.width, safeZone.height);
            
            // Draw safe zone border
            this.ctx.strokeStyle = isPlayerClan ? '#00ff00' : clanColor;
            this.ctx.lineWidth = 2;
            this.ctx.setLineDash([10, 5]);
            this.ctx.strokeRect(screenPos.x, screenPos.y, safeZone.width, safeZone.height);
            ...
            this.ctx.fillText(`ZONA ${clanName}`, screenPos.x + safeZone.width / 2, screenPos.y + 20);
        });
    }
    ...
    drawPlayer() {
        ...
        LogManager.log('info', `Drawing player at world (${this.player.x.toFixed(0)}, ${this.player.y.toFixed(0)}) -> screen (${screenPos.x.toFixed(0)}, ${screenPos.y.toFixed(0)})`);
        ...
        this.ctx.fillText(displayName, screenPos.x + this.player.width / 2, screenPos.y - 10);
    }
    ...
    drawRaidIndicator(player) {
        if (player.id !== 'player') {
            ...
            this.ctx.fillRect(centerX - healthBarWidth / 2, screenPos.y - 25, healthBarWidth, healthBarHeight);
            
            this.ctx.fillStyle = '#00ff00';
            this.ctx.fillRect(centerX - healthBarWidth / 2, screenPos.y - 25, healthBarWidth * healthPercentage, healthBarHeight);
        }
    }
    ...
    drawEnemy(enemy) {
        ...
        this.ctx.fillStyle = '#00ff00';
        this.ctx.fillRect(screenPos.x, screenPos.y - 10, enemy.width * healthPercent, 5);
    }
    
    drawBullet(bullet) {
        LogManager.log('debug', '[Disparo P5] drawBullet: Renderizando la bala en el canvas.', { bulletX: bullet.x, bulletY: bullet.y });
    ...
    drawObstacle(obstacle) {
        ...
        if (obstacle.type === 'base') {
            ...
            this.ctx.textAlign = 'center';
            this.ctx.fillText('BASE', screenPos.x + obstacle.width / 2, screenPos.y + obstacle.height / 2 + 5);
    ...
    drawMinimap() {
        ...
        this.coinObjects.forEach(coin => {
            ...
            ctx.fillRect(
                minimapX + coin.x * scale,
                minimapY + coin.y * scale,
                1, 1
            );
        });
        ...
        if (this.centralSafeZone) {
            ...
            const offsetY = minimapY + (this.worldSize / 2) * scale - spriteHeight / 2;
    ...
    drawCoin(coin) {
        ...
        this.ctx.textAlign = 'center';
        this.ctx.fillText('$', screenPos.x + coin.width / 2, screenPos.y + coin.height / 2 + 2);
    }
    ...
    initializeMultiplayer() {
        ...
        const botNames = [
            ...
            "Yankee", "Zulu", "Ace", "King", "Queen", "Jack", "Spade", "Heart",
            "Diamond", "Club", "Star", "Moon", "Sun", "Earth", "Mars", "Jupiter"
        ];
        ...
        for (let i = 0; i < this.maxPlayers - 1; i++) {
            ...
            LogManager.log('info', `Bot ${bot.name} assigned to clan: ${this.clans[bot.clan].name} (index: ${bot.clan})`);
        }
    ...
    createBot(name, index) {
        ...
        } while (spawnX < 0 || spawnX > this.worldSize - 40 || 
                 spawnY < 0 || spawnY > this.worldSize - 40);
        
        // Determinar el tipo de bot (Jefe, Elite o Común)
        const rand = Math.random();
        let isBoss = false;
        let isElite = false;
        if (rand < 0.05) { // 5% de ser Jefe
            isBoss = true;
        } else if (rand < 0.75) { // 70% de ser Elite (del 5% al 75%)
            isElite = true;
        }
        // El 25% restante son comunes
        
        // Debug: Log bot spawn position
        LogManager.log('info', `Bot ${name} (${isBoss ? 'JEFE' : isElite ? 'ELITE' : 'NORMAL'}) spawning at (${spawnX.toFixed(0)}, ${spawnY.toFixed(0)}) for clan ${clan.name}`);
    ...
    initializeWorld() {
        ...
        this.obstacles = [];
        for (let i = 0; i < 200; i++) { // More obstacles for larger map
            let tries = 0;
            let valid = false;
            let obs = null;
    ...
    spawnEnemyForClan(clanIndex) {
        ...
        } while (x < 0 || x > this.worldSize || y < 0 || y > this.worldSize || 
    ...
    isInAnySafeZone(x, y, excludeClan = null) {
        return this.clanSafeZones.some(safeZone => {
            ...
            return x >= safeZone.x && x <= safeZone.x + safeZone.width &&
    ...
    isPlayerInSafeZone(player) {
        ...
        // Zonas de clan
        const playerSafeZone = this.clanSafeZones.find(zone => zone.clan === player.clan);
    ...
    updateCamera() {
        ...
        if (!this.player || !isFinite(this.player.x) || !isFinite(this.player.y)) {
            LogManager.log('warn', 'Invalid player coordinates in updateCamera:', this.player);
            return;
        }
        ...
        if (!isFinite(newCameraX) || !isFinite(newCameraY)) {
            LogManager.log('warn', 'Invalid camera coordinates calculated:', { newCameraX, newCameraY, playerX: this.player.x, playerY: this.player.y });
            return;
        }
        ...
        if (!this.lastCameraLog || now - this.lastCameraLog > 5000) {
            LogManager.log('info', `Camera at (${this.camera.x.toFixed(0)}, ${this.camera.y.toFixed(0)}) following player at (${this.player.x.toFixed(0)}, ${this.player.y.toFixed(0)})`);
            this.lastCameraLog = now;
        }
        
        // Final validation
        if (!isFinite(this.camera.x) || !isFinite(this.camera.y)) {
            LogManager.log('error', 'Camera coordinates became invalid after bounds check:', this.camera);
            this.camera.x = 0;
            this.camera.y = 0;
        }
    }
    ...
    worldToScreen(worldX, worldY) {
        ...
        if (!isFinite(worldX) || !isFinite(worldY) || 
            ...
            LogManager.log('warn', 'Invalid coordinates in worldToScreen:', { worldX, worldY, cameraX: this.camera.x, cameraY: this.camera.y });
            return { x: 0, y: 0 };
        }
        ...
        if (!this.lastWorldToScreenLog || now - this.lastWorldToScreenLog > 5000) {
            LogManager.log('info', `World (${worldX.toFixed(0)}, ${worldY.toFixed(0)}) -> Screen (${screenX.toFixed(0)}, ${screenY.toFixed(0)}) [Camera: (${this.camera.x.toFixed(0)}, ${this.camera.y.toFixed(0)})]`);
    ...
    update() {
        if (this.gameState === 'playing' && this.player) {
            ...
            if (!this.lastUpdateLog || now - this.lastUpdateLog > 5000) {
                LogManager.log('info', `Updating game - Player at (${this.player.x.toFixed(0)}, ${this.player.y.toFixed(0)})`);
            ...
            localStorage.setItem('inCentralSafeZone', inCentralSafeZone ? 'true' : 'false');

            // Reset mouse clicked state after processing
            this.mouse.clicked = false;
        } else {
            LogManager.log('warn', `Update skipped - GameState: ${this.gameState}, Player: ${!!this.player}`);
        }
    }
    ...
    gameLoop() {
        ...
        if (!this.lastGameLoopLog || now - this.lastGameLoopLog > 5000) {
            LogManager.log('info', `=== GAME LOOP RUNNING === GameState: ${this.gameState}, FPS: ${PerformanceOptimizer.deviceCapabilities.fps}, Mode: ${PerformanceOptimizer.deviceCapabilities.performanceMode}`);
    ...
    drawCentralSafeZone() {
        ...
        this.ctx.fillStyle = '#ffffff';
        this.ctx.textAlign = 'center';
        this.ctx.fillText('ZONA SEGURA NEUTRAL', screenPos.x + zone.width / 2, screenPos.y + 30);
        this.ctx.restore();
    }
        ...
        for (let y = 0; y < sprite.length; y++) {
            for (let x = 0; x < sprite[0].length; x++) {
                ...
                if (val) {
                    ...
                    const screen = this.worldToScreen(offsetX + x * pixel, offsetY + y * pixel);
            ...
            for (let y = 0; y < sprite.length; y++) {
                for (let x = 0; x < sprite[0].length; x++) {
                    ...
                    if (val) {
                        const color = colors[val];
                        this.ctx.fillStyle = color;
                        this.ctx.fillRect(offsetX + x * pixel, offsetY + y * pixel, pixel, pixel);
                        this.ctx.strokeStyle = '#888';
                        this.ctx.strokeRect(offsetX + x * pixel, offsetY + y * pixel, pixel, pixel);
                    }
                }
            }
    ...
    spawnSatellite() {
        ...
        do {
            x = Math.random() * this.worldSize;
            y = Math.random() * this.worldSize;
        } while (this.getDistance(x, y, this.earth.x, this.earth.y) < this.earth.radius + 200);
    ...
    drawPlanetPortals() {
        ...
        this.planetPortals.forEach(portal => {
            ...
            const isOnScreen = screenPos.x > -100 && screenPos.x < this.canvas.width + 100 && 
            ...
            this.ctx.beginPath();
            this.ctx.arc(screenPos.x, screenPos.y, currentRadius + 10, 0, Math.PI * 2);
            this.ctx.fill();
            this.ctx.restore();
            ...
            this.ctx.font = 'bold 14px Arial';
            this.ctx.textAlign = 'center';
            this.ctx.fillText(portal.name, screenPos.x, screenPos.y + currentRadius + 20);
            
            // Dibujar descripción
            this.ctx.fillStyle = '#cccccc';
            this.ctx.font = '10px Arial';
            this.ctx.fillText(portal.description, screenPos.x, screenPos.y + currentRadius + 35);
            ...
            if (this.player) {
                const distance = this.getDistance(this.player.x, this.player.y, portal.x, portal.y);
                if (distance < currentRadius + 50) {
                    // Mostrar indicador de interacción
                    this.ctx.fillStyle = '#00ff00';
                    this.ctx.font = 'bold 16px Arial';
                    this.ctx.textAlign = 'center';
                    this.ctx.fillText('CLICK PARA ENTRAR', screenPos.x, screenPos.y - currentRadius - 30);
                    ...
                    if (this.mouse.clicked) {
                        ...
                        const mouseDistance = this.getDistance(mouseWorld.x, mouseWorld.y, portal.x, portal.y);
                        
                        if (mouseDistance < currentRadius + 20) { // Aumentar área de click
                            console.log(`Portal clicked: ${portal.name} at distance ${mouseDistance}`);
    ...
    forceRegisterExistingWidgets() {
        ...
        const elementsToRegister = [
            { id: 'tiendaBtn', selector: '#tiendaBtn', name: 'Botón de Tienda', pos: { x: window.innerWidth / 2 - 100, y: 80 } },
            { id: 'playerStats', selector: '.player-stats', name: 'Estadísticas del Jugador', pos: { x: 10, y: 10 } },
            { id: 'widgetInstructions', selector: '#widgetInstructions', name: 'Instrucciones de Widgets', pos: { x: 20, y: window.innerHeight - 200 } },
            { id: 'widgetControlPanel', selector: '#widgetControlPanel', name: 'Control de Widgets', pos: { x: 20, y: 80 } },
            { id: 'resetWidgetsButton', selector: '#resetWidgetsButton', name: 'Botón Resetear', pos: { x: 200, y: 20 } }
        ];
